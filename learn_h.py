...
def get_rollouts(env, policy, args, n_batch_rollouts, policy1,lin=None):
    rollouts = []
    if lin is None:
        ...
        for i in range(n_batch_rollouts):
            ...
            rollouts.extend(policy.main(args, env, lime = lin, flag=1, tt=policy1))
    return rollouts
...
if __name__ == '__main__':
    ...
    for j in range(5,51):
        ...
        for i in range(max_iters):
            ...
            #ff.write('\n')
            #grade[i][j-23] =precision[-1]
            record = np.array(serials_train[1:10])
            ff=open('lime_hotdash.txt','a+')
            np.savetxt('lime_hotdash.txt',record)
            ff.write('\n')
            ff.close()
            student_trace = get_rollouts(env=net_env, policy=student, args=args, n_batch_rollouts=n_batch_rollouts,policy1=teacher,lin=lin)
    ...
    #pool.close()
    #pool.join()
    #np.savetxt('data.txt',grade,fmt='%.5f')
    #best_lin = trees[-1]
