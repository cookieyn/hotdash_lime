...
class RobustMPC:
    ...
    def predict(self, state):
        ...
        #print (np.where(state[0, -1] * np.max(VIDEO_BIT_RATE) == VIDEO_BIT_RATE)[0])
    ...
    def predict_fn(self,serial_state):
        state = zhenghe(serial_state)
        #print('hello')
        #print (state.shape)
        tongji = [0,0,0,0,0,0]
        for i in range(6):
        ...
        for full_combo in CHUNK_COMBO_OPTIONS:
            ...
            reward = (bitrate_sum / 1000.) - (REBUF_PENALTY * curr_rebuffer_time) - (smoothness_diffs / 1000.)
                ...
                #print (combo[0])
            if combo != ():
                tongji[combo[0]] = reward
        for i in range (6):
            tongji[i] -=  min(tongji)
        #print (tongji)
        if sum(tongji) !=0:
            for i in range (6):
                tongji[i] = tongji[i] / sum(tongji)
        else:
            tongji = 1/6
        
        return tongji
        
def zhenghe(serial_state):
    state = np.zeros((5,8))
    state[0,-1] = serial_state[0]
    state[1,-1] = serial_state[1]
    state[2,-1] = serial_state[2]
    for i in range(5):
        state[3,i] = serial_state[3+i]
    state[4,-1] = serial_state[8]
    #print (serial_state)
    return state
