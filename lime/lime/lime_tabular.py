...
class TableDomainMapper(explanation.DomainMapper):
    ...
    def visualize_instance_html(self,
        ...
        ''' % (exp_object_name, json.dumps(out_list, ensure_ascii=False),
               label, div_name)
        return ret
    ...
    def explain_instance(self,
        ...
        if train_local_error is not None:
            data, inverse = self.__data_inverse(
                data_row, num_samples, derivation=train_local_error)
        else:
...
class RecurrentTabularExplainer(LimeTabularExplainer):
    ...
    def __init__(self,
        ...
        training_data = np.transpose(
            training_data, axes=(0, 2, 1)).reshape(n_samples,
