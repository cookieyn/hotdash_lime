...
if __name__ == '__main__':
    ...
    for i in range(max_iters):
        ...
        # cur_states, cur_actions, cur_serials = np.array(states), np.array(actions), np.array(serials)
        ...
        # print('Prune begins...')
        #
        # prune_tree = ccp.prune(tree=dt_policy, x_test=serials_val, y_test=actions_val, max_leaf_nodes=args.leaf_nodes)
        # dt_policy = prune_tree

        # reward = student.main(args, policy=dt_policy)
        reward = 0
    ...
    # out_graph.write_svg('decision_tree/' + args.abr + '.svg')
